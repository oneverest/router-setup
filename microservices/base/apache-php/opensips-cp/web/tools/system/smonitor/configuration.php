 ...
 */
 ...
 if ($_POST['set']!=null)
 {
  ...
  {
   $sql =  "UPDATE ".$table." SET extra = ? WHERE name='sampling_time' AND box_id = ?";
   $stm = $link->prepare($sql);
  ...
  $sql = "UPDATE ".$table." SET extra = ? WHERE name='chart_size' AND box_id = ?";
  $stm = $link->prepare($sql);
  if ($stm->execute(array($chart_size, $box_id)) === false)
  	die('Failed to issue query, error message : ' . print_r($stm->errorInfo(), true));
 
  if ($chart_history=="auto") $chart_history_value="auto";
  $sql = "UPDATE ".$table." SET extra = ? WHERE name='chart_history' AND box_id = ?";
  $stm = $link->prepare($sql);
