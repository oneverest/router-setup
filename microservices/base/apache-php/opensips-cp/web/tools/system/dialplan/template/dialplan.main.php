...
$has_record_command = substr($sql_command, 0, strpos($sql_command, 'order by'));
$stm = $link->prepare("select count(*) ".$has_record_command);
if ($stm===FALSE) {
	die('Failed to issue query [select count(*) '.$has_record_command.'], error message : ' . $link->errorInfo()[2]);
}
